...
class Fold{
	...
	public function __construct(){
		...
		add_action( 'customize_register', array('Fold_Customizer' , 'register') );
		add_action( 'customize_preview_init', array('Fold_Customizer' , 'live_preview') );
	...
	public function setup() {
		...
		load_theme_textdomain( 'fold', get_template_directory() . '/languages' );
		...
		$starter_content = array(
			...
			'nav_menus' => array(
				...
				'header-right' => array(
					'name' => __( 'Bottom of Header - Right', 'fold' ),
					'items' => array(
						'link_youtube',
						'link_github',
						'link_linkedin',
		...
		add_image_size( 'fold' . '-featured-image', 2000, 1200, true );
		add_image_size( 'fold' . '-thumbnail-avatar', 90, 90, true );
	...
	public function widgets_init() {
		...
		register_sidebar( array(
			'name'          		=> esc_html__( 'Footer Column 1', 'fold' ),
			'id'            		=> 'footer-column-1',
			'description'   		=> esc_html__( 'Add widgets here to appear in your footer column 1.', 'fold' ),
		...
		register_sidebar( array(
			'name'          		=> esc_html__( 'Footer Column 2', 'fold' ),
			'id'            		=> 'footer-column-2',
			'description'   		=> esc_html__( 'Add widgets here to appear in your footer column 2.', 'fold' ),
			'before_widget' 	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'  	=> '</div>',
			'before_title'  	=> '<h4 class="widget-title">',
			'after_title'   		=> '</h4>',
		...
		register_sidebar( array(
			'name'          		=> esc_html__( 'Footer Column 3', 'fold' ),
			'id'            		=> 'footer-column-3',
			'description'   		=> esc_html__( 'Add widgets here to appear in your footer column 3.', 'fold' ),
			'before_widget' 	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'  	=> '</div>',
			'before_title'  	=> '<h4 class="widget-title">',
			'after_title'   		=> '</h4>',
		...
		register_sidebar( array(
			'name'          		=> esc_html__( 'Footer Column 4', 'fold' ),
			'id'            		=> 'footer-column-4',
			'description'   		=> esc_html__( 'Add widgets here to appear in your footer column 4.', 'fold' ),
			'before_widget' 	=> '<div id="%1$s" class="widget %2$s">',
			'after_widget'  	=> '</div>',
			'before_title'  	=> '<h4 class="widget-title">',
			'after_title'   		=> '</h4>',
	...
	public function excerpt_more( $link ) {
		...
		$link = sprintf( '<p class="link-more"><a href="%1$s" class="more-link btn btn-default" title="%2$s" data-toggle="tooltip" data-placement="bottom" aria-hidden="true"><span class="fa fa-eye"></span> ' . esc_html__( 'Continue reading', 'fold' ) . '</a></p>',
		...
		$fields   =  array(
			'author' => '<div class="form-group has-feedback comment-form-author">
								<div class="input-group">
									...
									<input placeholder="' . esc_attr__( 'Name', 'fold' ) .( $req ? ' *' : '' ) . '" class="form-control" id="author" name="author" type="text" value="' . esc_attr( $commenter['comment_author'] ) . '" size="30" required="required" data-error="' . esc_html__('Please enter your name!', 'fold') . '"' . $aria_req . ' />
								</div>
			...
			'email'  => '<div class="form-group has-feedback comment-form-email">
								<div class="input-group">
									...
									<input placeholder="' . esc_attr__( 'Email', 'fold' ) . ( $req ? ' *' : '' ) . '" style="direction: ltr;" class="form-control" id="email" name="email" type="email" value="' . esc_attr(  $commenter['comment_author_email'] ) . '" size="30" required="required" data-error="' . esc_html__('Please enter your email address!', 'fold') . '"' . $aria_req . ' />
								</div>
								<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
								<div class="help-block with-errors"></div>
							</div>',
			'url'    => '<div class="form-group has-feedback comment-form-url">
								<div class="input-group">
									...
									<input placeholder="' . esc_attr__( 'Website', 'fold' ) . '" style="direction: ltr;" class="form-control" id="url" name="url" type="url" value="' . esc_attr( $commenter['comment_author_url'] ) . '" size="30" data-error="' . esc_html__('Please enter a valid website starting with http:// on nothing!', 'fold') . '" />
								</div>
								<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
								<div class="help-block with-errors"></div>
							</div>',
		...
		$args['comment_field'] = '
											<div class="form-group has-feedback comment-form-comment">
												<div class="input-group">
													...
													<textarea placeholder="' . esc_attr__( 'Comment', 'fold' ) . '" class="form-control" id="comment" name="comment" cols="45" rows="8" required="required" data-error="' . esc_html__('Please enter your comment!', 'fold') . '"></textarea>
												</div>
												<span class="glyphicon form-control-feedback" aria-hidden="true"></span>
												<div class="help-block with-errors"></div>
											</div>';
	...
	public static function posts_pagination( $args = array() ) {
		...
		if ( $GLOBALS['wp_query']->max_num_pages > 1 ) {
			$args = wp_parse_args( $args, array(
				'mid_size'           => 1,
				'prev_text'          => esc_html__( 'Previous', 'fold' ),
				'next_text'          => esc_html__( 'Next', 'fold' ),
			) );
	...
	public static function posted_on() {
		global $wpp_settings;
		$time_string = '<time class="entry-date published" title="'. esc_html__('Posted on', 'fold') .'" data-toggle="tooltip" data-placement="bottom" datetime="%1$s">%2$s</time>';
		...
		<span class="posted-on">
			<i class="fa fa-calendar" aria-hidden="true" title="<?php esc_attr_e('Posted on', 'fold'); ?>" data-toggle="tooltip" data-placement="bottom"></i> 
	...
	public static function modified_on() {
		$time_string = '<time class="entry-date updated" title="' . esc_html__('Updated on', 'fold') .'" data-toggle="tooltip" data-placement="bottom" datetime="%1$s">%2$s</time>';
		$time_string = sprintf( $time_string,
		...
		<span class="modified-on">
			<i class="fa fa-pencil fa-lg" aria-hidden="true" title="<?php esc_attr_e('Updated on', 'fold'); ?>" data-toggle="tooltip" data-placement="bottom"></i>
			<a href="<?php echo esc_url( get_permalink() ); ?>" rel="bookmark"><?php echo $time_string; // xss ok ?></a>
		</span>
		<?php
	}
...
// change exactly 'fold' to 'your-text-domain'
// change exactly Fold to Your_Theme_Classname